...
orlpath = "/Users/lit/Documents/博1/proj/[dataset]/orl_face"
...
def load_orl(k):
    ...
    train_label = zeros(40 * k)  # [0,0,.....0](共40*k个0)
    test_face = zeros((40 * (10 - k), 112 * 92))
    test_label = zeros(40 * (10 - k))
...
def PCA(data, r):
    ...
    data_mean = mean(data, 0)  # 对列求平均值
    A = data - tile(data_mean, (rows, 1))  # 将所有样例减去对应均值得到A
    C = A * A.T  # 得到协方差矩阵
    D, V = linalg.eig(C)  # 求协方差矩阵的特征值和特征向量
    V_r = V[:, 0:r]  # 按列取前r个特征向量
    V_r = A.T * V_r  # 小矩阵特征向量向大矩阵特征向量过渡
    for i in range(r):
        V_r[:, i] = V_r[:, i] / linalg.norm(V_r[:, i])  # 特征向量归一化
    ...
    for r in range(10, 41, 10):  # 最多降到40维,即选取前40个主成分（因为当k=1时，只有40维)
        ...
        for k in range(1, 10):
            ...
            for i in range(num_test):
                ...
                diffMat = data_train_new - tile(testFace, (num_train, 1))  # 训练数据与测试脸之间距离
                sqDiffMat = diffMat ** 2
